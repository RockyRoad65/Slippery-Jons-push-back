...
void cheesyDrive(double ithrottle, double iturn) {
	...
	if (turnInPlace) {
		...
		left_d= remappedTurn * fabs(remappedTurn);
		right_d = -remappedTurn * fabs(remappedTurn);
...
void move_drivetrain(uint8_t LEFT_FRONT_PORT, uint8_t LEFT_REAR_PORT, uint8_t LEFT_MIDDLE_PORT, uint8_t RIGHT_FRONT_PORT, uint8_t RIGHT_REAR_PORT, uint8_t RIGHT_MIDDLE_PORT, double ithrottle, double iturn) {
  ...
  motor_move(LEFT_FRONT_PORT, 127*left_d);
  motor_move(LEFT_REAR_PORT, 127*left_d);
  motor_move(LEFT_MIDDLE_PORT, 127*left_d);
  motor_move(RIGHT_FRONT_PORT, 127*right_d);
  motor_move(RIGHT_REAR_PORT, 127*right_d);
  motor_move(RIGHT_MIDDLE_PORT, 127*right_d);
